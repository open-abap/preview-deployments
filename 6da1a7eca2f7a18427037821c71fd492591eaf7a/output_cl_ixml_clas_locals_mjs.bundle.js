...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique40 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique40);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique41 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique41);
      ...
      let unique42 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique42++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique43 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique43++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), new abap.types.Integer().set(0))) {
      ...
      let unique45 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique45++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), new abap.types.Integer().set(0))) {
      ...
      let unique46 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique46++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique47++);
