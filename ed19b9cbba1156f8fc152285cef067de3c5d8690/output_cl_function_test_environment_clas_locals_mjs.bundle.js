"use strict";
(self["webpackChunkopen_table_maintenance"] = self["webpackChunkopen_table_maintenance"] || []).push([["output_cl_function_test_environment_clas_locals_mjs"],{
...
class lcl_input_arguments {
  ...
  async if_ftd_input_arguments$get_importing_parameter(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique165 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique165.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 19};
      throw unique165;
    }
  ...
  async if_ftd_input_arguments$get_table_parameter(INPUT) {
    ...
    abap.statements.readTable(this.mt_tables,{into: ls_row,
      ...
      withKeySimple: {"name": name}});
    if (abap.compare.ne(abap.builtin.sy.get().subrc, abap.IntegerFactory.get(0))) {
      const unique166 = await (new abap.Classes['CX_FTD_PARAMETER_NOT_FOUND']()).constructor_();
      unique166.EXTRA_CX = {"INTERNAL_FILENAME": "cl_function_test_environment.clas.locals_imp.abap","INTERNAL_LINE": 28};
      throw unique166;
    }
    result.set(ls_row.get().value);
    return result;
...
class lcl_invoker {
  ...
  static async invoke(INPUT) {
    ...
    for await (const unique167 of abap.statements.loop(lo_result.get().mt_exporting)) {
      ls_exporting.set(unique167);
      fminput.importing[ls_exporting.get().name.get().toLowerCase().trimEnd()].set(ls_exporting.get().value.dereference());
    }
    for await (const unique168 of abap.statements.loop(lo_result.get().mt_tables)) {
      ls_table.set(unique168);
